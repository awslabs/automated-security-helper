...
if __name__ == "__main__":
    ...
    if args.output:
        ...
        if ashignore_path.exists():
            ...
            print(
                cyan(f"Imported ash-ignore-report.txt from {args.output}"),
                file=sys.stderr,
            )
        if ashscanset_path.exists():
            ...
            print(
                cyan(f"Imported ash-scan-set-files-list.txt from {args.output}"),
                file=sys.stderr,
            )

    if not ashignore_content:
        ashignore_content = get_ash_ignorespec_lines(
            args.source, args.ignorefile, debug=args.debug
        )
    ...
    if not ashscanset_list:
        ...
        ashscanset_list = get_files_not_matching_spec(
            args.source, spec, debug=args.debug
        )
    ...
    if args.output:
        if not ashignore_imported:
            debug_echo(
                f"Writing ash-ignore-report.txt to {args.output}", debug=args.debug
            )
        ...
        if not ashscanset_imported:
            debug_echo(
                f"Writing ash-scan-set-files-list.txt to {args.output}",
                debug=args.debug,
            )
