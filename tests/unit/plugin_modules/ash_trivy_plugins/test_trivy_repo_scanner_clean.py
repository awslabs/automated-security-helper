...
class TestTrivyRepoScannerConfig:
    ...
    def test_config_options_validation(self):
        ...
        config = TrivyRepoScannerConfig(
            options=TrivyRepoScannerConfigOptions(scanners=["vuln", "misconfig"])
        )
    ...
    def test_scanner_initialization_with_custom_config(
        ...
        scanner = TrivyRepoScanner(
            context=mock_plugin_context, config=custom_trivy_config
        )
...
class TestTrivyRepoScannerConfigProcessing:
    ...
    def test_process_config_options_default(
        self, mock_plugin_context, default_trivy_config
    ):
        """Test processing of default configuration options."""
        scanner = TrivyRepoScanner(
            context=mock_plugin_context, config=default_trivy_config
        )
        ...
        scanners_arg = next(
            (arg for arg in extra_args if arg.key == "--scanners"), None
        )
        ...
        license_arg = next(
            (arg for arg in extra_args if arg.key == "--license-full"), None
        )
        ...
        ignore_arg = next(
            (arg for arg in extra_args if arg.key == "--ignore-unfixed"), None
        )
        ...
        telemetry_arg = next(
            (arg for arg in extra_args if arg.key == "--disable-telemetry"), None
        )
...
class TestTrivyRepoScannerDependencies:
    ...
    @patch(
        "automated_security_helper.plugin_modules.ash_trivy_plugins.trivy_repo_scanner.find_executable"
    )
    ...
    def test_dependencies_satisfied_flag(
        self, mock_find_executable, mock_plugin_context
    ):
        ...
        assert (
            not hasattr(scanner, "dependencies_satisfied")
            or not scanner.dependencies_satisfied
        )
...
class TestTrivyRepoScannerErrorHandling:
    ...
    def test_subprocess_execution_failure(
        self,
        mock_find_executable,
        mock_plugin_context,
        mock_target_directory,
        ...
        with (
            ...
            patch.object(
                scanner,
                "_run_subprocess",
                side_effect=ScannerError("Subprocess failed"),
            ),
        ):
    ...
    def test_argument_resolution_error(
        self,
        mock_find_executable,
        mock_plugin_context,
        mock_target_directory,
    ...
    def test_pre_scan_scanner_error_propagation(
        self, mock_plugin_context, mock_target_directory
    ):
        ...
        with patch.object(
            scanner, "_pre_scan", side_effect=ScannerError("Pre-scan failed")
        ):
