...
class TestScanWorkflowIntegration:
    ...
    async def test_scan_progress_detection_with_partial_results(
        ...
        # Create scanners directory structure with multiple target types
        scanners_dir = output_directory / "scanners"
        scanners_dir.mkdir(exist_ok=True)

        # Create scanner1 with both source and converted targets
        scanner1_dir = scanners_dir / "scanner1"
        scanner1_dir.mkdir()
        ...
        with open(source_dir / "ASH.ScanResults.json", "w") as f:
            json.dump(source_result, f)

        # Converted target
        converted_dir = scanner1_dir / "converted"
        converted_dir.mkdir()
        converted_result = {
            "findings": [
                {
        ...
        with open(converted_dir / "ASH.ScanResults.json", "w") as f:
    ...
    async def test_scan_with_malformed_result_files(
        ...
        valid_result = {
            "findings": [{"id": "1", "severity": "CRITICAL", "scanner": "scanner1"}]
        }
        with open(source_dir / "ASH.ScanResults.json", "w") as f:
    ...
    async def test_scan_with_missing_required_fields(
        ...
        """Test handling of result files with missing required fields."""
        # Get the scan registry
        registry = get_scan_registry()
        ...
        # Create scanner with results missing required fields
