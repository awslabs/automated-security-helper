...
class GrypeScanner(ScannerPluginBase[GrypeScannerConfig]):
    ...
    def validate_plugin_dependencies(self) -> bool:
        ...
        if not found:
            ASH_LOGGER.warning(f"Grype executable not found in PATH. Please ensure grype is installed.")
            ASH_LOGGER.info("You can install grype using: curl -sSfL https://raw.githubusercontent.com/anchore/grype/main/install.sh | sh -s -- -b /usr/local/bin")
    ...
    def scan(
        ...
        try:
            ...
            ASH_LOGGER.debug(f"Grype results structure: runs count = {len(grype_results.get('runs', []))}")
            ...
            try:
                ...
                if not sarif_report.runs:
                    ...
                    from automated_security_helper.schemas.sarif_schema_model import Run, Tool, ToolComponent
                    sarif_report.runs = [Run(
