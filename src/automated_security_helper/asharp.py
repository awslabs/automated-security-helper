...
cliparser = argparse.ArgumentParser(
    description="Automated Security Helper Aggregated Report Parser"
)
cliparser.add_argument(
    "-i", "--input", help=f"File contained ASH aggregated results ({DEF_INFILE})"
)
...
cliparser.add_argument(
    ...
    "--retain",
    action="store_true",
    help="TEMPORARY - Do not modify raw data output",
)
cliparser.add_argument(
    "-s", "--stdout", action="store_true", help="Output ARP resulting model to console"
)
cliparser.add_argument(
    "-v", "--verbose", action="store_true", help="Output instrumentation log"
)
...
def NormalizeJson(tool, file, data):
    ...
    # normalize bandit results
        ...
        for metric in metrics_raw:
            metrics.append(
                {"file": metric.replace(file, ""), "metrics": metrics_raw[metric]}
            )
...
def ParseAshSections(aggfile, aggregate_file_path):
    ...
    for section in sections:
        ...
        for subsection in subsections:
            ...
            if not gmodel:
                debug(
                    f"-- {subsection[0]} model is {len(subsection[2])} and did not detect json"
                )
                continue
        ...
        for m in models:
            ...
            try:
                ...
                #
                aggregated[provider]["data"] = aggregated[provider]["data"].replace(
                    data, ""
                )
            ...
            debug(
                f'-- {tool} model starts with "{data[0]}", contains {len(data)} bytes, and ends with "{data[:-5]}"'
            )

        # place the extacted json objects into the provider data model
        if arr:
            aggregated[provider]["model"] = arr
