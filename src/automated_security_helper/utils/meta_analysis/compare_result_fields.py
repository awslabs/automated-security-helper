...
def compare_result_fields(original_result: Dict, aggregated_result: Dict) -> List[Dict]:
    ...
    for path in orig_paths:
        ...
        # Check if the field exists in the aggregated result
        result = get_value_from_path(aggregated_result, path)

        if not result["exists"] and not is_expected_transformation:
            # Get the value from the original result
            orig_value_result = get_value_from_path(original_result, path)
            orig_value = orig_value_result["value"]
