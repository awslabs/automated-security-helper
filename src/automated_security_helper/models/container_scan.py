...
class ContainerVulnerability(BaseFinding):
    ...
    layer_id: Optional[str] = Field(None, description="ID of the affected container layer")
    fixed_version: Optional[str] = Field(None, description="Version that fixes the vulnerability")
    base_image_name: Optional[str] = Field(None, description="Base image name if vulnerability is from base image")
...
class ContainerScanReport(BaseModel):
    ...
    definition: Annotated[str, Field(description="Definition of the container image, e.g. the Dockerfile that created it.")] = None
