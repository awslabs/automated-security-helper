    ...
    steps:
      ...
      - name: Set up Python
        ...
        with:
          python-version: '3.10'
      - name: Install ASH
        run: pip install git+https://github.com/awslabs/automated-security-helper.git@v3.2.7
      - name: Run ASH scan
        run: ash --mode local
...
ash-scan-container:
  ...
  script:
    ...
    - pip3 install git+https://github.com/awslabs/automated-security-helper.git@v3.2.7
    - ash --mode container
  artifacts:
    paths:
      - .ash/ash_output
...
pipeline {
    ...
    stages {
        stage('Install ASH') {
            steps {
                sh 'pip install git+https://github.com/awslabs/automated-security-helper.git@v3.2.7'
            }
        }
        stage('Run ASH Scan') {
            steps {
    ...
    stages {
        stage('Install ASH') {
            steps {
                ...
                sh 'pip3 install git+https://github.com/awslabs/automated-security-helper.git@v3.2.7'
            }
        }
        stage('Run ASH Scan') {
            steps {
...
jobs:
  scan:
    ...
    steps:
      ...
      - run:
          name: Install ASH
          command: pip install git+https://github.com/awslabs/automated-security-helper.git@v3.2.7
      - run:
          name: Run ASH scan
...
> **Tip**: The CI examples in this guide use pinned versions (`@v3.2.7`) for reproducibility. You can also use the `v3` floating tag (`@v3`) to always get the latest stable v3.x release, though pinned versions are recommended for CI/CD pipelines.
