...
We maintain a `v3` Git tag that always points to the latest stable v3.x release. This means you can use `@v3` in your installation commands instead of a specific version like `@v3.2.4`:

```bash
alias ash="uvx git+https://github.com/awslabs/automated-security-helper.git@v3"
```

This is convenient for local development where you always want the latest version. For CI/CD pipelines or environments where reproducibility matters, we recommend pinning to a specific release tag (e.g., `@v3.2.4`).
