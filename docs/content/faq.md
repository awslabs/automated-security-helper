...
alias ash="uvx git+https://github.com/awslabs/automated-security-helper.git@v3.5.8"

# Using pipx
pipx install git+https://github.com/awslabs/automated-security-helper.git@v3.5.8

# Using pip
pip install git+https://github.com/awslabs/automated-security-helper.git@v3.5.8
```

### What are the prerequisites for ASH v3?

### What is the `v3` floating tag?
We maintain a `v3` Git tag that always points to the latest stable v3.x release. This means you can use `@v3` in your installation commands instead of a specific version like `@v3.5.8`:

```bash
alias ash="uvx git+https://github.com/awslabs/automated-security-helper.git@v3"
```

This is convenient for local development where you always want the latest version. For CI/CD pipelines or environments where reproducibility matters, we recommend pinning to a specific release tag (e.g., `@v3.5.8`).
   ...
   ```json
   {
     "mcpServers": {
       "ash": {
         "command": "uvx",
         "args": [
           "--from=git+https://github.com/awslabs/automated-security-helper@v3.5.8",
           "ash",
           "mcp"
         ],
         "disabled": false,
...
3. **Test the MCP server**: Try running the server directly:
   ```bash
   uvx --from=git+https://github.com/awslabs/automated-security-helper@v3.5.8 ash mcp --help
   ```

### How do I test the ASH MCP server?
```bash
# Test MCP server startup
uvx --from=git+https://github.com/awslabs/automated-security-helper@v3.5.8 ash mcp --debug

# Check ASH version
uvx --from=git+https://github.com/awslabs/automated-security-helper@v3.5.8 ash --version
```
