...
> **Tip**: The examples below use pinned versions (`@v3.5.7`) for reproducibility. You can also use the `v3` floating tag (`@v3`) to always get the latest stable v3.x release, though pinned versions are recommended for CI/CD.
    ...
    steps:
      ...
      - name: Set up Python
        ...
        with:
          python-version: '3.10'
      - name: Install ASH
        run: pip install git+https://github.com/awslabs/automated-security-helper.git@v3.5.7
      - name: Run ASH scan
        run: ash --mode local
