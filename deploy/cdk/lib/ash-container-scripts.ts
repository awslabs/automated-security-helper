...
# Ask this image's ASH which MCP options it accepts, once.
#
# This script is baked into the image, but WHICH ASH the image contains comes
# from the AshVersion parameter at deploy time, so the script and the binary can
# disagree. They currently do: neither --stateless-http nor --allowed-host exists
# in any ASH release up to and including v3.7.0, nor on main or beta. Passing an
# unknown option makes ASH's Typer parser exit non-zero, so the server never
# binds a port and the deployment fails as a health-check timeout twenty minutes
# after an image build that succeeded — never naming the flag.
#
# COLUMNS is set wide on purpose. Typer renders help through rich, which wraps to
# the terminal width; at a default width a long paired flag can be split across
# lines and a supported option would then measure as missing.
...
#
# WHEN THE OPTION IS ABSENT AND STATELESS WAS ASKED FOR, THIS REFUSES TO START.
# Omitting it quietly would leave a STATEFUL server running, which is the
# behaviour of every ASH release that predates the option — and is exactly what
# the option exists to prevent. AgentCore generates an Mcp-Session-Id and injects
# it into the request, and a stateful server answers 404 to a session it never
# issued, so every request fails and the failure reads as a client bug. Behind a
# load balancer with more than one replica the same thing happens on whichever
# replica did not see the handshake. Either way the container would report
# healthy while answering nothing, so it is better not to start.
...
#
# WHY THIS WARNS WHERE THE STATELESS CHECK REFUSES. The asymmetry is deliberate.
# An operator can opt out of stateless with McpStatelessHttp=false, so refusing
# leaves them a way forward. There is no equivalent opt-out here: the Fargate
# stack substitutes the load balancer's DNS name whenever McpAllowedHost is left
# empty, so on that target ASH_MCP_ALLOWED_HOST is never unset and a refusal
# would be unsatisfiable — no parameter value would let the stack deploy. The
# degradation is also narrower: an ASH without the option accepts any Host, which
# is that ASH's only behaviour rather than a regression from it. So this is loud
# in the container log and continues.
