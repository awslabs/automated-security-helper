...
# encrypted under it.
#
# WHY ONE KEY FOR BOTH LOG GROUPS RATHER THAN ONE EACH
# ---------------------------------------------------
...
# single log group ARN; here the image build log and the gate's scan log sit inside
# one trust boundary -- the same operators read both, and both carry ASH scan and
# configuration detail for the same repository -- so a second key would buy no
# isolation and would double a standing charge.
#
...
resource "aws_kms_key" "this" {
  ...
  description = "Encrypts the ASH pull-request gate's CloudWatch log groups and CodeBuild output."

  # Nothing outside this module reads the gate's logs or the image build output, so
  # a rotation needs no coordination, and CloudWatch Logs keeps decrypting data
