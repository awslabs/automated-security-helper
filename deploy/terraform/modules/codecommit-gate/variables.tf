...
variable "kms_key_arn" {
  description = <<-EOT
    ARN of an existing customer managed KMS key to encrypt this module's two log
    groups and its CodeBuild output with, instead of the key it creates.

    Leave it null and the module creates its own key. There is no third option:
    both log groups are always under a customer managed key. The gate's log holds
    the findings it is about to post on a pull request, which is scan output for
